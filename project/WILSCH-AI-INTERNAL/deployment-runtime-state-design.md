...
- **Credentials** — split into config (git) and secrets (GitHub Secrets). `.env.config` is committed to git and contains non-sensitive configuration (`LIBRECHAT_PORT=3082`, `MODEL_NAME=gpt-4o`). GitHub Secrets (repo-level) holds credentials (`OPENROUTER_KEY`, `MONGO_PASSWORD`). At deploy time, the GHA reusable workflow assembles a `.env` file by merging `.env.config` with matched secrets from `.env.example` (the schema), then ships it to the server via `scp`. The deploy script runs `docker compose up` — Compose reads `.env` natively. `.env.example` serves as the machine-readable schema: keys with values = config (goes in `.env.config`), keys without values = secrets (injected from GitHub Secrets). Adding a new secret follows a ceremony: `gh secret set` + commit the key to `.env.example` — the issue-commit-linker captures the schema change as the audit trail. GitHub Team plan org audit log (180-day retention) provides who/when for secret value changes. **Resolved (Phase A):** Agent has ACL-based read/write access to `.env` files under `/home/deploy/projects/` on both WILSCH-AI-SERVER and IITR-STAGING. Mechanism: `setfacl -m u:agent:rw` on existing `.env` files (explicit), `setfacl -R -d -m u:agent:rw` on project directories (default inheritance — new files auto-inherit `user:agent:rw-`). Breaks read-only invariant for `.env` only. **Ceremony:** (1) Agent reads `.env.example` in the repo as schema (keys with values = config, keys without = secrets), (2) agent receives credential values from user (chat or local `.env` file), (3) agent writes `.env` on server via SSH, (4) agent commits `.env.example` to git for audit trail (issue-commit-linker captures the schema change). This ceremony is for initial project setup during migration — ongoing deploys use GHA. **Undefined (Phase B):** GHA assembly (`.env.config` + GitHub Secrets → assembled `.env` → scp, off-the-shelf via [SpicyPizza/create-envfile](https://github.com/SpicyPizza/create-envfile) + [appleboy/scp-action](https://github.com/appleboy/scp-action)) or [psst](https://github.com/Michaelliv/psst) (AI-native secrets manager — agents use secrets without seeing them). Phase B restores the security invariant by removing agent credential visibility.
...
**Undefined:** Docker DNS collision on shared infra networks. When multiple app composes (staging + preview) run simultaneously, services with the same name on the shared infra network collide via Docker DNS round-robin. `COMPOSE_PROJECT_NAME` only isolates the default network, not shared external networks. Evidence: #1306 — staging and preview both registered `pipelines` on `iitr-staging-infra`, causing intermittent Masterfragen filter failures. The fix was connection-string-level (explicit container name instead of DNS alias), not network membership restriction. Additionally, volume cloning copies staging's connection strings into previews, which may point to staging services instead of the preview's own. Needs a design pass to determine the convention: minimize network membership, use explicit addressing, or a combination.
...
**Port convention — default 0, `.env` overrides.** Compose files default published ports to 0 (`${LIBRECHAT_PORT:-0}:3080`). Without a `.env`, Docker auto-assigns an ephemeral port — safe for previews, broken for staging. Staging and local dev `.env` set fixed ports matching their routing (`LIBRECHAT_PORT=3082`). The preview deploy script needs no override — 0 is already the compose default. The script discovers the assigned port via `docker compose port` and generates the Caddy config. Two layers: compose defaults to 0, `.env` sets real ports where needed. `.env.example` declares which port vars must be set. Evidence: #1326 — staging `.env` had `PORT=0`, Docker assigned ephemeral 32836, Caddy proxied to fixed 3082 — staging unreachable for 4 days. **Undefined:** enforcement mechanism for port convention — spike needed. The Level 0 validator checks: at least one service has `deploy.entry`. Zero = fail (no entry point declared).
