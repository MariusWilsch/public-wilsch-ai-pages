...
**Undefined:** Proof point ownership split — the Developer should own defining WHAT the proof point is, WHAT the gemba is (how to test it), and WHAT constitutes "proven." This is the spike equivalent of tracking.md: the Developer writes the success criteria (which primitives to test, expected behavior per primitive, what "proven" means) and the Dev Lead witnesses against them. In #1167 (first spike ceremony), the Dev Lead created the success criteria table (Steps 0–6) during the ceremony because the Developer's tracking.md only contained pass/fail — Finding #1 explicitly called this out: "tracking.md contains only pass/fail criteria results. No documentation of tool evaluation." The Developer should have defined the gemba; the Dev Lead should have only witnessed it. Evidence: [#1167 Finding 1](https://github.com/DaveX2001/deliverable-tracking/issues/1167#issuecomment-4079492662), CCI #605 and #648 observations on proof point witnessing and question-level attribution.
